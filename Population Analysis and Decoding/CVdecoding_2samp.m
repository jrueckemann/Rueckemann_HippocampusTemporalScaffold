function [postprob,LL,entropy,maxIdx,...
    ...
    CVdecoding_2samp(trlrte1,trlrte2,n_cv,bayesdist,n_pseudotrl,samp_meth,rotate_data,cortest,coriter)
%
...
if nargin<6 || isempty(samp_meth)
    samp_meth=struct('SampleRatio',0.5,'SampleReplacement',false,...
        'CondSpecific',true);
end
if nargin<7 || isempty(rotate_data)
    rotate_data=false;
end
if nargin<8 || isempty(cortest)
    cortest=false;
end
if nargin<9 || isempty(coriter)
    coriter=0;
end


%Preallocate data splits
n_unit=numel(trlrte1);
n_cond=cellfun(@numel,trlrte1);
n_cond=unique(n_cond);
assert(numel(n_cond)==1,'Inconsistent number of conditions in each neuron')
trlsplit1=repmat(cell(1,1,n_cond),n_unit,1);
trlsplit2=repmat(cell(1,1,n_cond),n_unit,1);
...
for n=1:n_unit %Iterate across neurons
    for c=1:n_cond %Iterate across conditions
        trlsplit1{n}{c}=randsampling(trlrte1{n}{c},n_cv,samp_meth);
        trlsplit2{n}{c}=randsampling(trlrte2{n}{c},n_cv,samp_meth);
    end
end
...
for s=1:n_cv
    ...
    for n=1:n_unit
        for c=1:n_cond
            curtrlidx=trlsplit1{n}{c}(s,:);
            train_trl{n}{c}=trlrte1{n}{c}(:,curtrlidx); %training data

            testtrlidx=trlsplit2{n}{c}(s,:);
            test_trl{n}{c}=trlrte2{n}{c}(:,testtrlidx); %test data
        end
    end
