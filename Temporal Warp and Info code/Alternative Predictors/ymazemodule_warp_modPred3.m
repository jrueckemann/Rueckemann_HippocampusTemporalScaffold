function [spkstruct,Pstruct,err,errmsg,errID]=ymazemodule_warp_modPred3(unitstruct,evtdata,timetemplate,iter,savefolder,saveplotdata,varargin)
%
%Jon Rueckemann 2024
...
addParameter(p,'modendevt',false,@(x) islogical(x)); %trial end event comes from the next trial
...
for s=1:n_spk
    spkstruct(s).Y_warpLindist=[];
    spkstruct(s).Y_warpLindist_Remap=[];
end
...
for m=1:n_mazes
    ...
    lindist=[evtdata{m}.xydata(:,1)./1000 evtdata{m}.lindist];
    [predID,trlidx,validwarp,warpts,knotmap,epochmap,predmap]=...
        warptrial_fulltrial4info_lindist3(...
        evtmatrix,timetemplate,binctr,lindist);
    predID=predID(:);
    trlidx=trlidx(:);
    if all(isnan(trlidx))
        badmaze(m)=true;
    ...
    evtprefix=struct('Task','Ymaze','EventName','Y_warpLindist');
    ...
    for s=1:n_spk
        if isempty(spkstruct(s).Y_warpLindist) %1st valid maze
            spkstruct(s).Y_warpLindist=curevt_Yinfo(s);
        else
            spkstruct(s).Y_warpLindist(m)=curevt_Yinfo(s);
        end
    end
end
...
if ~all(badmaze)
    estruct=struct('EventName','Y_warpLindist','WarpTimeBins',warpts);
    for s=1:n_spk
        ...
        spkstruct(s).Y_warpLindist_Remap=rstruct;
    end
end
...
if ~isempty(savefolder)
    ...
    savefilename=['WarpYmazeLindistMC_' daylabel '.mat'];
